...
public partial class c_login : System.Web.UI.Page
{
    ...
    protected void submit_Click(object sender, EventArgs e)
    {
        ...
        {
            //手机验证码登录
            string txt_username = usermoblie.Value;
            string txt_userpwd = dynamiccode.Value;
        }
        else
        {
            string txt_username = Request["username"].Trim();
            string txt_userpwd = Request["password"].Trim();
            string sql = "select * from jz_manager where username='" + txt_username + "'";
            DataTable dt = DAL.SQLHelper.GetDtBySql(sql);
            if (dt.Rows.Count > 0)
            {
                string A = md5(txt_userpwd, 32);
