...
! function ($) {
    function CalendarPrice(t) {
        t.el || t.error && t.error({
            code: 1,
            msg: CODES[1]
        }), this.calendar = $(t.el), this.settingWindow = createSettingWindow(), this.opts = $.extend({}, DEFAULTS,
            t), this.init()
    }
    var formatNumber = function (t) {
        ...
        isValid = function (t) {
            return !!/^(\d{4})[-\/\.](\d{1,2})[-\/\.](\d{1,2})/.test(t) && RegExp.$1 + "/" + formatNumber(RegExp.$2) +
                "/" + formatNumber(RegExp.$3)
        },
    ...
    fn._formatThisMonth = function (t) {
        var e = null;
        return (e = /^(\d{4})[-\.\/](\d{1,2})/.test(t) ? this.dateToObject(RegExp.$1 + "/" + RegExp.$2) : this.dateToObject(
            formatDate(TODAY, "yyyy/MM"))) <= this.endDate && e >= this.startDate ? e : this.dateToObject(
            formatDate(this.startDate, "yyyy/MM"))
    }, fn.init = function () {
        this.createStyleCode(), this.startDate = this._getStartDate(), this.endDate = this._getEndDate(), this.data =
            this._getOptionsData(), this.month = this._formatThisMonth(this.opts.month), this.initSettingWindow(),
            this.createCalendar(), this.handleClickEvent()
        ...
        var t = this.opts.endDate,
            ...
            a = null,
            r = null;
        return /^(\d{4})[-\.\/](\d{1,2})/.test(t) && (e = RegExp.$1, a = toNumber(RegExp.$2)), e && a || (e =
                toNumber(formatDate(TODAY, "yyyy")) + 1, a = toNumber(formatDate(TODAY, "MM"))), r =
            /^\d{4}[-\.\/]\d{1,2}[-\.\/](\d{1,2})/.test(t) ? RegExp.$1 : new Date(Date.parse(new Date(e, a)) -
                ONE_DAY_MSEC).getDate(), this.dateToObject(e + "/" + a + "/" + r)
    }, fn.dateToObject = function (t) {
        var e = "";
        return /(\d{4})[-\/\.](\d{1,2})[-\/\.]?/.test(t) && (e += RegExp.$1 + "/" + RegExp.$2),
            /[-\/\.]\d{1,2}[-\/\.](\d{1,2})/.test(t) ? e += "/" + RegExp.$1 : e += "/01", /\d{4}\/\d{1,2}\/\d{1,2}/
            .test(e) ? new Date(e) : (this.opts.error({
                code: 2,
        ...
        var r = "";
        r += '<div class="capricorncd-calendar-container">', r += '\t<div class="calendar-head-wrapper">', t && (r +=
                '       <a class="prev-month" title="上一月"></a>'), r += '\t\t<div class="calendar-month-title">' +
            formatDate(this.month, "yyyy年MM月") + "</div>", e && (r +=
                '       <a class="next-month" title="下一月"></a>'), r += "\t</div>", r +=
            '\t<div class="calendar-table-wrapper">', r += '  \t    <table cellpadding="4" cellspacing="0">', r +=
            '\t\t    <thead><tr class="week"><th class="weekend">日</th><th>一</th><th>二</th><th>三</th><th>四</th><th>五</th><th class="weekend">六</th></tr></thead>',
            r += "\t\t    <tbody>" + this._createTbody() + "</tbody>", r += "\t    </table>", r += "    </div>",
            this.opts.hideFooterButton || (r += '    <div class="calendar-foot-wrapper">', r +=
                '        <button class="btn btn-reset">重置</button>', r +=
                '        <button class="btn btn-confirm">确定</button>', r +=
                '        <button class="btn btn-cancel">取消</button>', r += "    </div>"), r += "</div>", this.calendar
            .html(r), this.renderDataToTalbe()
    }, fn._createTbody = function () {
        for (var t = this._getMonthDays(), e = this.month.getDay(), a = 0, r = Math.ceil((t + e) / 7), n = "", o =
                "", c = formatDate(this.startDate, "yyyy-MM-dd"), d = formatDate(this.endDate, "yyyy-MM-dd"), i = 0; i <
            r; i++) {
            o += "<tr>";
            for (var l = 1; l <= 7; l++)(a = 7 * i + l - e) > 0 && a <= t ? (n = formatDate(this.month, "yyyy-MM-") +
                    formatNumber(a), today == n && (a = "今天"), tomorrow == n && (a = "明天"), o += n >= c && n <= d ?
                    '<td class="valid-hook" data-week="' + (l - 1) + '" data-id="' + n + '"><b>' + a +
                    '</b><div class="data-hook"></div></td>' : '<td class="disabled"><b>' + a + "</b></td>") : o +=
                "<td> </td>";
            o += "</tr>"
        }
        return o
        ...
        var t = "";
        t += '<div class="cddsw-container">', t += '   <div class="cddsw-head-wrapper">', t +=
            '       <div class="cddsw-title">0000-00-00</div>', t += '       <a class="cddsw-close"><i></i></a>', t +=
            "   </div>", t += '   <ul class="cddsw-form-wrapper clearfix">', t += this._createDaySetupInputGroup(),
            t += "   </ul>", t += '   <fieldset class="cddsw-batch-settings clearfix">', t +=
            '       <legend class="bs-title"><b>批量设置</b></legend>', t += '       <div class="bs-content">', t +=
            '           <lable class="bs-lable">日期范围</lable>', t += '           <div class="bs-options-wrapper">',
            t += '               <input class="itext" name="startDay" type="text">', t +=
            '               <span class="white-space">-</span>', t +=
            '               <input class="itext" name="endDay" type="text">', t +=
            '               <label class="drw-enable"><input name="enableDateRange" type="checkbox"> 启用</label>', t +=
            "           </div>", t += "       </div>", t += '       <div class="bs-content bs-week-chekbox">', t +=
            '           <lable class="bs-lable">设置星期</lable>', t += '           <div class="bs-options-wrapper">',
            t += '               <label><input name="setWeek" type="checkbox" value="1"> 周一</label>', t +=
            '               <label><input name="setWeek" type="checkbox" value="2"> 周二</label>', t +=
            '               <label><input name="setWeek" type="checkbox" value="3"> 周三</label>', t +=
            '               <label><input name="setWeek" type="checkbox" value="4"> 周四</label>', t +=
            '               <label><input name="setWeek" type="checkbox" value="5"> 周五</label>', t +=
            '               <label><input name="setWeek" type="checkbox" value="6"> 周六</label>', t +=
            '               <label><input name="setWeek" type="checkbox" value="0"> 周日</label>', t +=
            "           </div>", t += "       </div>", t += "   </fieldset>", t +=
            '   <div class="cddsw-foot-wrapper">', t += '       <button class="btn-confirm">启用本设置</button>', t +=
            '       <button class="btn-cancel">取消</button>', t += "   </div>", t += "</div>", this.settingWindow.html(
                t), $("body").append(this.settingWindow)
    }, fn._createDaySetupInputGroup = function () {
        for (var t = "", e = this.opts.config, a = 0; a < e.length; a++) {
            var r = e[a];
            t += "<li>", t += "   <label>" + r.name + "</label>", t += '   <input name="' + r.key +
                '" type="text">', t += "</li>"
        }
        return t
        ...
        this.calendar.on("click", ".prev-month", function () {
            ...
            t.opts.everyday ? t.opts.everyday(a) : (t.settingWindow.find(
                '.cddsw-form-wrapper [type="text"]').val(""), t.settingWindow.find(
                '[name="enableDateRange"]').prop("checked", !1), t.settingWindow.find(
                '[name="setWeek"]').prop("checked", !1), $.each(a, function (e, a) {
                t.settingWindow.find('[name="' + e + '"]').val(a)
            }), t.settingWindow.find(".cddsw-title").html(e), t.settingWindow.find(
                '[name="startDay"], [name="endDay"]').val(e), t.settingWindow.show())
            ...
            if (d) {
                ...
                l = t.handleSetDateRangeData(formatDate(t.startDate, "yyyy-MM-dd"), formatDate(t.endDate,
                    "yyyy-MM-dd"))
            }
        ...
        for (var a = [], r = Date.parse(this.dateToObject(t)), n = Date.parse(this.dateToObject(e)), o = Math.floor(
                (n - r) / ONE_DAY_MSEC) + 1, c = 0; c < o; c++) {
        ...
        return r ? r < today ? (this.opts.error({
            ...
            code: 7,
            msg: CODES[7]
        }), null) : (r == n ? a.push(formatDate(this.dateToObject(r), "yyyy-MM-dd")) : a = this._createDateRangeArr(
            r, n), a) : (this.opts.error({
            code: 6,
            msg: CODES[6]
        ...
        if (count) {
            var defaultStyle = {
                ...
                reg = null;
            for (var key in concatStyle) reg = eval("/{" + key + "}/g"), templateStyle = templateStyle.replace(reg,
                concatStyle[key]);
        ...
        for (var e = 0, a = null, r = 0; r < t.length; r++) {
            e = r;
            for (var n = r + 1; n < t.length; n++) t[n].date < t[e].date && (e = n, a = t[r], t.splice(r, 1, t[n]),
                t.splice(n, 1, a))
        }
        return t
