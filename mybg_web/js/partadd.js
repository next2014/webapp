	...
	view.addEventListener('pageBeforeBack', function(e) {
		if(e.detail.page.id == 'salaryhtml') {
			if(document.getElementById('salary').value) {
			    ...
			    document.getElementById('salary_typetext').value = document.getElementById('salary').value + $('.salary_typename')[0].innerHTML;
	...
	$.ready(function () {
	    if (typeof timetypeData != "undefined") {
			...
			timetypePickerBtn.addEventListener('tap', function(event) {
				...
				timetypePicker.show(function (items) {
				    descriptiontext.value = items[0].value;
		...
		if(sdatePicker) {
			...
			sdatePicker.addEventListener('tap', function() {
				...
				picker.show(function (rs) {
				    //mui.toast(rs.text);
				    descriptionsdate.value = rs.text;
					sdate.value = rs.text;
		...
		if(edatePicker) {
			...
			edatePicker.addEventListener('tap', function() {
				...
				var picker = new $.DtPicker(options);
				picker.show(function (rs) {
				    descriptionedate.value = rs.text;
					edate.value = rs.text;
		...
		deadComPicker.addEventListener('tap', function() {
			...
			var picker = new $.DtPicker(options);
			picker.show(function (rs) {
			    descriptiondeadline.value = rs.text;
